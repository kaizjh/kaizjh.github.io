...
本课程侧重于抽象的概念，Function Abstraction 和 Data Abstraction，是程序员能够不受限于一些底层的细节，所以本课程以Python为主要授课语言。但是本课程并没有限定程序员们应该使用哪种语言来编程，因为一旦我们掌握了编程的基本概念，那么学习一门新的编程语言应该不是一件难事，而且，我们还在本课程中学习了Scheme和SQL两种语言。
本课程印象最深刻的一个课后作业是，[用Python编写一个Scheme编译器](https://github.com/kaizjh/cs61a/tree/main/scheme)，从中我了解到，编译器是一个程序，输入一些值，进入程序计算，输出计算结果。最核心的是程序计算，也是我们现在要做的主要工作，根据识别输入值得到的关键词进行分类，其中主要有表达式、函数，很多编译器有两个互相嵌套的函数（互递函数），一个处理表达式，一个处理函数，经过一系列的计算得到结果再输出。